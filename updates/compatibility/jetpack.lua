if mods["jetpack"] then
    ...
    --Equivalent to vanilla tier 3
    --Tier 3 original recipe: 2 jetpack 2, 30 LDS, 30 blue circuits
    local pack2_recipe = data.raw.recipe["jetpack-2"]
    pack2_recipe.ingredients = {
        {"jetpack-1", 2},
        {"cube-resplendent-plate", 20},
        {"cube-arcane-drive", 20}, --arcane drive already contains the other 2 ingredients so this feels a little redundant, but it fits with the pattern already established
        {"cube-spectral-processor", 20}
    }
    pack2_recipe.category = craft_category

    local pack2_tech = data.raw.technology["jetpack-2"]
    pack2_tech.unit.ingredients = {techs[1], techs[2], techs[3], techs[4], techs[5]}
    ...
    --Equivalent to vanilla tier 4
    --I want this to have 4 thrust so that the max thrust is the same as it is with the regular jetpack mod, but thrust is hardcoded for each jetpack
    --The only way to make this 4 thrust is by using the jetpack 4 item prototype and pretend it's tier 3
    local pack3_recipe = data.raw.recipe["jetpack-4"]
    pack3_recipe.ingredients = {
        {"jetpack-2", 4}, --this may seem extreme, but since a tier was removed this is still a similar cost to the original tier 4
    ...
    --everything else uses the jetpack 4 prototypes so that the thrust is correct and the original jetpack 3 is pruned by disable_incompatible.lua
    local pack3_tech = data.raw.technology["jetpack-3"]
    pack3_tech.effects[1].recipe = "jetpack-4"
    pack3_tech.unit.count = 400
    pack3_tech.unit.ingredients = {techs[1], techs[2], techs[3], techs[4], techs[5], techs[6]}
